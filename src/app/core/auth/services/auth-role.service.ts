...
export class AuthRoleService {
  ...
  setInitPermission() {
    if (this.isRole('super_admin')) {
      const _permissions = this.local.getItem<IPermissionActions>('permissions');
  ...
  setResetPermission() {
    ...
    if (this.isRole('super_admin')) this.local.removeItem('currentCenter');
    if (this.isRole('super_admin')) this.setPermissionGroup();
  }

  setPermissionByRole(role: Extract<TRole, 'center_manager'>) {
    if (role === 'center_manager' && this.isRole('super_admin')) this.setPermissionCenterInfo();
    if (role === 'center_manager' && this.isRole('super_admin')) this.setPermissionCenter();
  }

  canCenterLogin() {
    if (
      (this.isRole('super_admin') || this.isRole('super_admin')) &&
  ...
  private setPermissionAdmin(): void {
    const _permissions = this.local.getItem<IPermissionActions>('permissionsAll');
  ...
  private setPermissionCenter(): void {
    ...
    ModulesOfMgrCenter.forEach((m) => (_cPermissions[m] = _permissions[m] || []));
  ...
  private setPermissionCenterInfo(): void {
    ...
    ModulesOfMgrCenter.forEach((m) => (_cPermissions[m] = (_permissions[m] || []).filter((p) => p === 'list')));
